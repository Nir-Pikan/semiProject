...
import clientGui.PopUp;
import clientIO.CardReaderController;
...
/** a class for the card reader system */
public class CardReaderSystem implements Initializable {
	...
	/**
	 * sets the {@link CardReaderController}
	 * 
	 * @param cardReaderController instance of {@link CardReaderController}
	 */
	...
	void ReadCardID(ActionEvent event) {
		...
		try {
			...
			PopUp.showInformation("Wrong input", "Wrong input", "Please enter appopriate data");
			return;
		}

		if (numberOfPeople < numberOfSubsribers) {
			PopUp.showInformation("Wrong input", "Wrong input", "Number of subscribers is larger then visitors");
			return;
		}
		if (numberOfPeople == 0) {
			PopUp.showInformation("Wrong input", "Wrong input", "Number of visitors is 0");
			return;
		}
	...
	/** simulating a visitor entering the park */
	@FXML
	void VisitorEnterToThePark(ActionEvent event) {
		...
		cardReaderController.enterVisitor(selectedItem.getCard(), selectedItem.getNumberOfPeople(),
				selectedItem.getNumberOfSubscribers());
		// cardReaderController.enterVisitor(selectedItem.getCard()); //method
		// enterVisitor need to be implemented

	}

	/**
	 * simulating a visitor leaving the park
	 * <p>
	 * Card reader recognize that the visitor exits the park<br>
	 * and remove him from the TableView
	 **/
	@FXML
	...
	/** closing the {@link CardReaderSystem} */
	@FXML
