...
/** the waiting list controller class */
public class WaitingListController implements IController {
	 ...
	 * 
	 * @param order    instance of {@link OrderController}
	 * @param messageC instance of {@link MessageController}
	 * @param parkC    instance of {@link ParkController}
	 */
	...
	/** initialize the waiting list cleaner */
	...
	/** Create the waitingList table in DB if not exists */
	private void createTable() {
		// please consider that type (enum value) name was changed from IDtype to type,
		// hope no bug is will be found
		boolean isCreated = dbController
				.createTable("waitingList(parkSite varchar(20),numberOfVisitors int,orderID int,"
	...
	/**
	 * Handle Cancel will move status to SEMICANCELED<br>
	 * and will wait for response or will cancel
	 * 
	 * @param canceled the {@link Order} that could be canceled
	...
	private void handelCancel(Order canceled, WakeableThread thread) {
		...
		while ((nextWaiting = getNextOrder(canceled)) != null) {
			...
			Platform.runLater(() -> {
				notifyWaitingOrder(orderToNotify);
			});
	...
	/**
	 * Adding new {@link Order} to waiting list
	 * 
	 * @param ord the {@link Order} that will be added to waiting list
	 * @return true if added successfully<br>
	 *         false otherwise
	 */
	...
	/**
	 * Notify the waiting {@link Order}
	 * 
	 * @param nextWaiting the next waiting {@link Order} to be notified
	 */
	private void notifyWaitingOrder(Order nextWaiting) {
		messageC.SendEmailAndSMS(nextWaiting.email, nextWaiting.phone,
				"Your Order No: " + nextWaiting.orderID + " can be accepted,\n"// content
						+ " Please Go to GoNature and approve/cancel this order." + "This offer is for one hour only!",
				"Order " + nextWaiting.orderID + " out of waiting list");// subject

	}

	/**
	 * Notify new {@link Order}
	 * 
	 * @param nextWaiting the new {@link Order} to be notified
	 */
	private void notifyNewOrder(Order nextWaiting) {
		messageC.SendEmailAndSMS(nextWaiting.email, nextWaiting.phone,
				"Your Order Added to the waiting list:\n" + "Order ID: " + nextWaiting.orderID + "\n" + "visit time: "
						+ nextWaiting.visitTime.toLocalDateTime().toLocalDate() + " "
						+ nextWaiting.visitTime.toLocalDateTime().toLocalTime().format(DateTimeFormatter.ISO_LOCAL_TIME)
						+ "\n" + "number of visitors: " + nextWaiting.numberOfVisitors + "\n" + "Price: "
						+ nextWaiting.priceOfOrder + "\n" + "Thank for using GoNature",
				"Order " + nextWaiting.orderID + " added to the waiting list");// subject

	}

	/**
	 * Delete {@link Order} from waiting list
	 * 
	 * @param order the {@link Order} to be deleted from waiting list
	 */
	...
	/**
	 * Get next {@link Order} and set to message sent
	 * 
	 * @param canceled the {@link Order} to be canceled
	 * @return the next {@link Order} waiting
	 */
	private synchronized Order getNextOrder(Order canceled) {
		...
		Timestamp minimum = Timestamp
				.valueOf(canceled.visitTime.toLocalDateTime().minusSeconds(TimeUnit.HOURS.toSeconds(AvgVisitTime - 1)));
		Timestamp maximum = Timestamp
				.valueOf(canceled.visitTime.toLocalDateTime().plusSeconds(TimeUnit.HOURS.toSeconds(AvgVisitTime)));
		PreparedStatement ps = dbController
				.getPreparedStatement("SELECT * FROM waitingList WHERE timestamp(visitTime) >= timestamp( ? ) "
		...
		for (Order o : list) {
			if (order.IsOrderAllowedWaitingList(o, canceled.numberOfVisitors)) {// setting the Order to sent message
				dbController.sendUpdate("UPDATE waitingList SET orderStatus='WAITINGLISTMASSAGESENT' WHERE orderID = "
						+ o.orderID + ";");
				return o;
			}
		}
	...
	/**
	 * Get waiting {@link Order}
	 * 
	 * @param orderId the id of the {@link Order} you want
	 * @return the {@link Order}<br>
	 *         false otherwise
	 */
