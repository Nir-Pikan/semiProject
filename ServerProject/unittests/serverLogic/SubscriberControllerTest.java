...
class SubscriberControllerTest {
	...
	@Test
	void testHandleRequestGetSubscriberWithOutCreditCard() {
		Subscriber expected = new Subscriber("S111111111","111111111","john","dou","0000000000","john@doe.com",1,Subscriber.Type.SUBSCRIBER);
		getSub(expected);
		
		String returned = subController.handleRequest(new ServerRequest(Manager.Subscriber,"GetSubscriberData","S111111111"));
	...
	@Test
	void testHandleRequestGetSubscriberWithCreditCard() {
		Subscriber expected = new Subscriber("S111111111","111111111","john","dou","0000000000","john@doe.com",1,Subscriber.Type.SUBSCRIBER);
	...
	@Test
	void testHandleRequestGetSubscriberNoData() {
		...
		try {
			...
			String actual = subController.handleRequest(new ServerRequest(Manager.Subscriber,"GetSubscriberData","S111111111"));
			assertEquals(expected, actual);
			verify(db,times(1)).sendQuery(anyString());
	...
	ResultSet getSub(Subscriber s) {
		...
		try {
			...
			when(db.sendQuery("SELECT * FROM subscribers WHERE subscriberID = \"" + s.subscriberID + "\";")).thenReturn(rsSub);
			...
			if(s.creditCardNumber!=null){
				...
				when(db.sendQuery("SELECT * FROM creditcards WHERE creditCardNumber = " + s.creditCardNumber + ";")).thenReturn(rsCredit);
			}
